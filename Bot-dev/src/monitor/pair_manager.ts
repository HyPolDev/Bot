...
export class PairManager {
    ...
    private calculateBidSweep(polyBids: any[], kalshiBids: any[], targetSize: number) {
        if (!polyBids || !kalshiBids || polyBids.length === 0 || kalshiBids.length === 0) return { size: 0 };
        
        let pIdx = 0; let kIdx = 0;
        const pBook = polyBids.map(l => ({ ...l }));
        const kBook = kalshiBids.map(l => ({ ...l }));

        let totalShares = 0;
        let polyRevenue = 0;
        let kalshiRevenue = 0;
        let polyWorstPrice = 1.0;
        let kalshiWorstPrice = 1.0;
        ...
        if (totalShares === 0) return { size: 0 };

        const blendedKalshi = kalshiRevenue / totalShares;
        const totalKalshiFees = Math.ceil(0.07 * totalShares * blendedKalshi * (1 - blendedKalshi) * 100) / 100;
        const kalshiWorstPriceCents = Math.floor(kalshiWorstPrice * 100);

        return {
            size: totalShares,
            polyVwap: polyRevenue / totalShares,
            kalshiVwap: kalshiRevenue / totalShares,
            polyWorstPrice,
            kalshiWorstPriceCents,
            totalKalshiFees
        };
    ...
    private async evaluateTakeProfit(openPos: any) {
        ...
        switch (openPos.type) {
            case 'PolyYes_KalshiNo':
                polyBids = this.latestPolyBook.yes?.bids || [];
                kalshiBids = this.latestKalshiBook.no?.bids || [];
                break;
            case 'PolyNo_KalshiYes':
                polyBids = this.latestPolyBook.no?.bids || [];
                kalshiBids = this.latestKalshiBook.yes?.bids || [];
                break;
        ...
        const sweep = this.calculateBidSweep(polyBids, kalshiBids, openPos.size);
        if (sweep.size > 0 && sweep.size >= openPos.size * 0.99) { // Can liquidate all or mostly all
            const netExitRevenue = sweep.polyVwap! + sweep.kalshiVwap! - (sweep.totalKalshiFees! / sweep.size);
            if (netExitRevenue >= 1.00) {
                logger.info(`[Take-Profit] TRIGGERED for ${this.pairId}! Net Revenue: ${netExitRevenue.toFixed(3)} >= 1.00`);
                
                const settings = await this.getSettings();
                this.lastArbitrageTime = Date.now();
                
                if (!settings.isPaperTrading) {
                    const polyAssetId = openPos.type.includes('PolyYes') ? this.polyYesTokenId : this.polyNoTokenId;
                    const kalshiSide = openPos.type.includes('KalshiYes') ? 'yes' : 'no';

                    this.liveEngine.queueOrder({
                        pairId: this.pairId,
                        marketQuestion: this.pairData.polyMarket.market_question,
                        tradeType: openPos.type,
                        targetSize: openPos.size,
                        ...
                        isEntry: false,
                        expectedEAR: 999, // Highest priority
                        availableLiquidity: sweep.size,
                        expiringDate: openPos.expiringDate
                    });
                } else {
                    this.portfolio.closePosition(this.pairId, sweep.size, sweep.polyVwap!, sweep.kalshiVwap!, sweep.totalKalshiFees!);
                    logger.info(`[Take-Profit] Paper traded exit for ${this.pairId}`);
                }
            }
        }
    }
    ...
    private async evaluateEntry() {
        ...
        const openPos = this.portfolio.getPosition(this.pairId);
        if (openPos) {
            await this.evaluateTakeProfit(openPos);
            return;
        }
        ...
        if (sweep.size > 0) {
            ...
            if (approvedSize > 0) {
                ...
                if (settings.isPaperTrading) {
                    ...
                    const polyAvailable = this.portfolio.getPolyCash() - maxEquityPerTrade;
                    const kalshiAvailable = this.portfolio.getKalshiCash() - maxEquityPerTrade;
                    
                    const polyRequiredCost = sweep.polyVwap * approvedSize;
                    const kalshiRequiredCost = sweep.kalshiVwap * approvedSize;

                    // Strictly check if we have physical cash before buffer, just for safety
                    if (this.portfolio.getPolyCash() < polyRequiredCost || this.portfolio.getKalshiCash() < kalshiRequiredCost) {
                        return;
                    }

                    if (polyAvailable < polyRequiredCost || kalshiAvailable < kalshiRequiredCost) {
                        logger.info(`[Buffer] Missed high EAR op on ${this.pairId} due to untouchable buffer.`);
                        if (typeof (this.portfolio as any).logMissedOpportunity === 'function') {
                            (this.portfolio as any).logMissedOpportunity(sweep.marginEAR);
                        }
                        return;
                    }
    ...
    private async executePaperEntry(
        ...
        if (realSweep.size > 0) {
            const maxEquityPerTrade = (settings as any).maxEquityPerTrade ?? 100;
            const polyRequiredCost = realSweep.polyVwap * realSweep.size;
            const kalshiRequiredCost = realSweep.kalshiVwap * realSweep.size;

            if ((this.portfolio.getPolyCash() - polyRequiredCost) < maxEquityPerTrade || (this.portfolio.getKalshiCash() - kalshiRequiredCost) < maxEquityPerTrade) {
                logger.info(`[Buffer] Paper missed high EAR op on ${this.pairId} due to buffer.`);
                if (typeof (this.portfolio as any).logMissedOpportunity === 'function') {
                    (this.portfolio as any).logMissedOpportunity(realSweep.marginEAR);
                }
                return;
            }
            ...
            if (opened) {
                ...
                }).catch((e: any) => logger.error(`[PairManager] Error persisting SimulatedTrade to DB: ${e.message}`));
            } else {
