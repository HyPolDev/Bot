...
export class PortfolioManager {
    ...
    public recentMissedOpportunities: { ear: number, timestamp: number }[] = [];

    public setManagers(managers: any[]) {
        this.registeredManagers = managers;
    }
    
    public getRegisteredManagers(): any[] {
        return this.registeredManagers;
    }
    
    public logMissedOpportunity(ear: number) {
        this.recentMissedOpportunities.push({ ear, timestamp: Date.now() });
        // Keep only last 10 minutes of missed opportunities
        const tenMinsAgo = Date.now() - 600000;
        this.recentMissedOpportunities = this.recentMissedOpportunities.filter(opp => opp.timestamp > tenMinsAgo);
    }
